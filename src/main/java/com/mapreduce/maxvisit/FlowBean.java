...
public class FlowBean implements Writable, Comparable<FlowBean> {
    ...
    public String toString() {
        return "phone=" + phone + ", upFlow=" + upFlow + ", downFlow=" + downFlow + ", amountFlow=" + amountFlow;
    }
    ...
    public int compareTo(FlowBean flow) {
        if ((flow.amountFlow - this.amountFlow) == 0) {
            return this.phone.compareTo(flow.phone);// 相等比较手机号，顺序比较
        }

        // 正常顺序 this.amountFlow-flow.amountFlow （从小到大）
        return flow.getAmountFlow() - this.amountFlow;// 倒序
    }
